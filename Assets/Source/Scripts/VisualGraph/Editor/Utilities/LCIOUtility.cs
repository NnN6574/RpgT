...
{
    public static class LCIOUtility
    {
        ...
        public static void Initialize(LcGraphView dsGraphView, string graphName)
        {
            ...
            _containerFolderPath = $"{PathConstants.PathRoot}{graphName}";
        ...
        public static void Save()
        {
            ...
            Debug.Log(PathConstants.PathConstructors + $"{_graphFileName}Graph");
            LCGraphSaveDataSO graphData = CreateAsset<LCGraphSaveDataSO>(PathConstants.PathConstructors, $"{_graphFileName}Graph");

            graphData.Initialize(_graphFileName);

            LCPlatformContainerSO platformContainer = CreateAsset<LCPlatformContainerSO>(_containerFolderPath, _graphFileName);
        ...
        private static void UpdateDialoguesChoicesConnections()
        {
            foreach (LCNode node in _nodes)
            {
                ...
                for (int choiceIndex = 0; choiceIndex < node.Choices.Count; ++choiceIndex)
                {
                    ...
                    if (nodeChoice.NodeID == Guid.Empty) continue;
        ...
        {
            LCGraphSaveDataSO graphData = LoadAsset<LCGraphSaveDataSO>(PathConstants.PathConstructors, _graphFileName);
        ...
        {
            foreach (KeyValuePair<Guid, LCNode> loadedNode in _loadedNodes)
            {
        ...
        {
            CreateFolder("Assets", "LC");
            CreateFolder("Assets/LC", "Graphs");
            CreateFolder("Assets/LC", "Platforms");
            CreateFolder("Assets/LC/Platforms", _graphFileName);
            CreateFolder(_containerFolderPath, "Global");
            CreateFolder(_containerFolderPath, "Groups");
            CreateFolder($"{_containerFolderPath}/Global", "Platforms");
        }
